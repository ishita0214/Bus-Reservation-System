...
export class BookingDetailsComponent {
  ...
  ngOnInit(): void {
    ...
    this.reservationService.currPage.subscribe((data) => {
      this.currPage = data;
  ...
  details: Details[] = []
  bookingId!: number
  userId: any = Number(localStorage.getItem('loginUser'))
  confirmedTickets: Details[] = []

  getDetails(bookingId: number) {
    this.reservationService.getPassengerDetails(bookingId).subscribe((data: Details[]) => {
      console.log(data);
      console.log(this.currPage);

      if (this.currPage == 'Upcoming') {
        data.forEach((temp) => {
  ...
  cancelTicket() {
    ...
    this.details.forEach((data) => {
      if (data.selected) {
        ...
        this.reservationService.updateReservation(data.reservation_id, reservation).subscribe(() => {
          console.log('Reservation cancelled:', data.reservation_id);

          this.details = [];
          this.getDetails(this.bookingId); // Refresh passenger details}
